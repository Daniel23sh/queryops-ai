...
```txt
M9 PR7 — V1 Live Validation, Manual QA, and Release Completion (active; readiness incomplete)
```
...
Milestone 6 is complete and merged into `main`. `M6 PR1 — Dashboards/Cards Backend Foundation`, `M6 PR2 — Dashboard/Card UI`, `M6 PR3 — CSV Export Backend`, `M6 PR4 — Card Refresh & CSV Export UI`, and `M6 PR5 — Card Reordering & Layout Persistence` plus the final Admin restricted-export policy are complete. PR #24 merged PR5. Milestone 7 — Product UX & Dashboard Redesign is complete. M7 PR1 is complete and merged through PR #25. M7 PR2 is complete and merged through PR #26. M7 PR3 — Dashboard Editor, Grid & Visualizations is complete and merged through PR #27. M7 PR4 — Ask Data Redesign & Final UX Hardening is complete and merged through PR #28. Milestone 8 — Actions, Approvals & Audit is complete and merged through PR #35; verified `main` reached `408190f1cdf5710ed80a83065d65fd9cd01c4f87`. M9 PR1 is complete and merged through PR #36 at `a21cdce59f7c3cd05e3e6fec72699554ffbb9979`. M9 PR2 is complete and merged through PR #37 at `800b2f4006057d7a046d28da8ddb28aebc2f6176`. M9 PR3 is complete and merged through PR #38 at `fd1b8cccba3190714233976daa334364c4b4b080`. M9 PR4 is complete and merged through PR #39 at `f8990b78e86de1d24a51783270e95fc05a07beca`. M9 PR5 is complete and merged through PR #40; verified `main` reached `695be1358ea2fcd67fc2cd25c66e2281986dd99f`. M9 PR6 is complete and merged through PR #41; verified `main` reached `c6691a204ccbb9eb007e2e0c6fe419c346745b13`, and its Backend, PostgreSQL Security, Frontend, E2E, M8 Primary E2E, and V1 Deterministic Release Gates GitHub checks passed. Milestone 9 — Evaluation, Quality Measurement & V1 Readiness is active. M9 PR7 is the explicitly activated release-validation follow-up. Mock remains the development and CI default, no live OpenAI measurement has been accepted as V1 release evidence, and the full manual QA checklist remains open; Milestone 9 and V1 remain incomplete until both pass.

M9 implementation followed this six-PR sequence: dataset/scoring foundation; runner/persistence; APIs/authorization; Evaluation UX; real-LLM evaluation mode; and V1 quality gates/readiness. PR7 is an explicitly activated evidence-focused release-validation follow-up, not a new feature phase.
...
M9 PR4 is merged through PR #39 at verified `main` commit `f8990b78e86de1d24a51783270e95fc05a07beca`.

M9 PR5 may add only one explicit opt-in OpenAI provider (`openai`) using the Responses API with strict structured output; centralized typed configuration and factory selection; minimized authorized prompt projection; sanitized provider errors and bounded measurements; governed Query Engine and evaluation-runner integration; manual CLI selection; safe Mock/OpenAI run metadata in the existing read API and workspace; documentation; and focused/full verification.

M9 PR5 guardrails:

* Mock remains the default for normal development, tests, and CI. An API key alone never activates OpenAI; real calls require explicit deployment configuration or `--provider openai`, and tests use injected fakes without external network access.
* Use only `gpt-5.6-terra` by default through the synchronous OpenAI Responses API with `store=false`, strict structured output, no tools, retrieval, streaming, background mode, or conversation state. Do not add a second real provider, framework, fallback, proxy/base URL, or browser provider selector.
* Provider prompts are explicit allowlist projections of the question, authorized queryable schema descriptions, approved business terms, scope type, and global-scope boolean. Never send identities, raw scope keys, permissions, rows, prior SQL, evaluator baselines/answers/case IDs, secrets, protected resources, or action targets.
* Provider SQL remains an untrusted candidate and must pass the existing SQL generator, authorization, validator, restricted runtime role, read-only limits, transaction-local context, and PostgreSQL RLS. Template-backed requests remain deterministic and provider-free.
* Persist only validated provider/model identity plus bounded duration/attempt/token measurements in existing sanitized JSON fields. Never persist or expose prompts, payloads, response IDs, headers, reasoning, generated/baseline SQL in Evaluation records, rows, keys, raw exceptions, or arbitrary SDK data; no migration or cost calculation is authorized.
* Evaluation remains sequential and manual. Keep one immutable provider/model per run, evaluator-only RLS-restricted baselines, semantic scoring, case isolation, and honest unsupported failures. Fatal configuration/authentication failures stop safely; ordinary case output failures remain visible.
* The role-aware read API may accept only validated `mock` or `openai` run metadata and select the latest eligible completed supported-provider run. Add no filter, history, comparison, picker, execution control, threshold, workflow, or expanded Manager disclosure.
* Do not change the dataset/templates to improve scores, Query Engine authorization/RLS semantics, actions/audit/notifications/exports, schema/migrations/seed/permissions, CI workflows, background infrastructure, or begin `M9 PR6 — V1 Quality Gates, Readiness & Completion`.

M9 PR5 delivered one explicit opt-in `openai` provider through the synchronous Responses API; centralized immutable configuration and provider construction; ambient SDK routing/header isolation; minimized authorized prompt projection; strict structured output and bounded safe failures/usage; governed Query Engine, evaluation runner, persistence, CLI, read-API, and read-only workspace integration; and network-free fake-provider verification. Mock remains the default, `OPENAI_API_KEY` alone never activates OpenAI, and template-backed queries remain provider-free.

Final verification passed 893 default backend tests with 153 expected PostgreSQL-only skips, all 1046 disposable-PostgreSQL tests with no skips, the exact 20-case M8 action-security suite, 268 frontend tests, both TypeScript checks, the production frontend build, five Evaluation Chromium flows, two isolated M8 primary/negative Chromium flows, Alembic head/no-diff checks, and focused provider/runner/API/CLI safety tests. The full deterministic MockLLM measurement remained 10/40 passed with security 4/5. No live OpenAI request was made because billable execution was not authorized. CodeRabbit was installed but unauthenticated, so the final review was **Manual M9 PR5 review — not a CodeRabbit result**; it fixed one Major ambient SDK configuration/proxy-isolation issue and left no actionable finding. No migration, second provider, fallback, browser execution/provider controls, real-provider workflow, threshold/cost policy, or M9 PR6 work was added.

M9 PR6 delivered the centralized `queryops-v1-readiness-v1` policy; strict full-run/current-dataset/result/usage validation; recomputed release metrics; explicit-run read-only CLI; parameter-free role-aware readiness API; bounded fail-closed Overview presentation; Ruff, scoped Pyright, compile, ESLint, explicit TypeScript, build, PostgreSQL, and E2E gates; a fail-closed aggregate CI job; and tracked quality, readiness, and manual-QA evidence. Actions and Dashboards remain `not_measured` in the frozen evaluation and map to deterministic PostgreSQL/browser evidence instead. Mock remains the default, and no real-provider call or secret exists in CI.

Final deterministic verification passed 98 focused readiness/API/CLI tests, 939 default backend tests with 153 expected PostgreSQL-only skips, all 1092 tests on a fresh disposable PostgreSQL 16 database with zero skips, the exact 20-case action-security suite, the tracked broader 30-case security matrix within the full suite, 274 frontend tests, Ruff, scoped Pyright, compileall, ESLint, both TypeScript checks, the production build, fresh Alembic head/no-diff checks, 12 general Chromium flows, and the isolated M8 primary and User-negative flows. CodeRabbit CLI 0.6.5 was installed but unauthenticated, so the final review was **Manual M9 PR6 security and release review — not a CodeRabbit result**; it fixed one Major fail-closed deterministic-evidence default and three Minor frontend/projection/documentation issues, with no remaining actionable finding.

M9 PR6 implementation is complete, but readiness is `incomplete`: no authorized live OpenAI smoke/full measurement was performed and the full manual QA checklist remains open. Do not mark Milestone 9 or QueryOps AI V1 complete until one eligible 40/40 OpenAI run passes every threshold and manual QA passes. No migration, dataset/baseline/template tuning, second provider/fallback, browser execution/settings/history, permission/RLS/runtime-role change, background infrastructure, billing/deployment work, or post-M9 scope was added.
...
PR7's current candidate adds semantic catalog v3; canonical `active_human_users`; primitive/composed directory-user concepts; deterministic mandatory/optional semantic grounding; a one-call structured `SemanticPlan` plus SQL contract; SQLGlot semantic conformance after the existing SQL safety validator on final sanitized SQL; no repair or second provider call; evaluation-stage classification; exact catalog/environment readiness identity; current-reference deterministic release seeding; filtered-run evidence preservation; and narrow one-cell aggregate alias normalization. PostgreSQL RLS remains authoritative. The current dataset is `it_operations_v1` v1 digest `1e7b12fbf35de4d2c52937a762f3960df444eb3303ee7061a0e4506819c22bc4`; the catalog is `it_operations_semantic_catalog` v3 hash `918df0c63288b7ed7ce700f8442c82bc1ffa3f51e1f4eaa63fd66012af82adb4`.

The sole baseline correction, `itops-medium-009`, is allowed because the tracked business term and deterministic query template already defined “non-compliant device” as non-compliant status, missing/outdated antivirus, or disabled encryption. Preserve that reviewed correction and its regression test; do not tune it from runtime scores. Do not change other frozen cases, questions, baselines, templates, readiness thresholds/policy ID, authorization, SQL/RLS/action boundaries, or browser execution surface.

Current deterministic candidate evidence is 375 focused Text-to-SQL tests, 1,100 default backend tests with 156 expected PostgreSQL-only skips, 1,236 disposable-PostgreSQL tests with zero skips, the exact 20-case action-security suite, 274 frontend tests, Ruff, zero-error/warning Pyright, ESLint, both TypeScript checks, production build, 12 general Playwright flows, two isolated M8 flows, healthy Compose services, and Alembic head/no-diff at `0010_disable_inactive_user`. Codex Security scan `f7036ba0-9e86-40c4-9e2d-0177908bbc31` completed all 17 changed runtime/configuration surfaces with zero findings. This candidate may be committed/frozen after final diff integrity and task-resource cleanup, but it is not V1-ready: no live OpenAI result or manual QA is accepted until an immutable candidate SHA exists, and readiness remains `incomplete`.
