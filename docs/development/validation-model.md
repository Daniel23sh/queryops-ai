...
| **V3** | Permission/RLS/grant/runtime-role change; migration; action execution; destructive/state-changing path; SQL safety/provider trust boundary; readiness/release logic; release candidate | Strong focused and adversarial regression evidence for every affected critical boundary, plus relevant broader subsystem checks. Actual release candidates run the complete authoritative release gates. | Use isolated disposable PostgreSQL with no relevant skips when PostgreSQL behavior, RLS, roles/grants, transactions, locking, persistence semantics, or database constraints are affected. Run migration/no-diff checks when schema, migrations, ORM metadata, or migration assumptions are affected. Browser/E2E is mandatory when the critical contract is exercised through a user-facing or governed end-to-end flow. | Independent review required, in addition to author self-review |
...
- **PostgreSQL:** Mandatory whenever correctness depends on roles, grants, RLS, transaction-local context, PostgreSQL SQL behavior, transactions, locks, concurrency, persistence semantics, or database-enforced constraints. A passing lightweight/default suite with PostgreSQL tests skipped is not sufficient.
...
- **Migrations:** Mandatory when schema, Alembic migrations, SQLAlchemy metadata, or migration assumptions are affected. Run `alembic upgrade head`, `alembic current`, and `alembic check` against a fresh disposable PostgreSQL database. For a changed migration, also verify its intended predecessor-to-head path and downgrade/upgrade round trip when downgrade is supported.
- **Browser:** Use component tests for local presentation logic. Playwright is mandatory when an affected critical contract is exercised through routing, authentication/session behavior, role-aware visibility, cross-layer requests, responsive interaction, or another governed end-to-end workflow.
- **CI and release:** Do not weaken or bypass existing jobs. Normal V3 engineering validation covers every affected critical boundary and does not require unrelated specialized categories. An actual release candidate must pass the complete authoritative release gates regardless of which boundary caused its V3 classification.
